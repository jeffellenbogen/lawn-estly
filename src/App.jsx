...
const CalibrationModal = ({ isOpen, onClose, onConfirm }) => {
...
export default function App() {
    ...
    if (file) {
      ...
      reader.onload = (evt) => {
        ...
        img.onload = () => {
            ...
            setView({ x: 0, y: 0, scale: 1 }); // Reset view
        };
  ...
  const startInteraction = (clientX, clientY) => {
    ...
    if (mode === 'CALIBRATING') {
      ...
      if (currentPoly.length > 2) {
        ...
        // Calculate distance visually on screen (accounting for zoom)
        const screenStart = {
  ...
  const endInteraction = (clientX, clientY) => {
     ...
     if (mode === 'CALIBRATING' && calibrationLine && !showCalibrationModal) {
        const rect = canvasRef.current.getBoundingClientRect();
        // Check drag distance visually
        const p1 = { 
            x: (calibrationLine.p1.x * rect.width * view.scale) + view.x, 
            y: (calibrationLine.p1.y * rect.height * view.scale) + view.y 
        };
        const p2 = { 
            x: (calibrationLine.p2.x * rect.width * view.scale) + view.x, 
            y: (calibrationLine.p2.y * rect.height * view.scale) + view.y 
        };
  ...
  // --- Modal Logic ---
  const confirmCalibration = (yards) => {
      if (!calibrationLine || !yards) return;
      const dist = getDistance(calibrationLine.p1, calibrationLine.p2);
      setScaleFactor(yards / dist);
      setShowCalibrationModal(false);
      setMode('IDLE');
  };
  ...
  useEffect(() => {
    ...
    // Draw Image
    // We draw the image at 0,0 with full width/height. 
    // The transform handles positioning it on screen.
    ctx.drawImage(image, 0, 0, width, height);

    // -- Drawing Logic uses NORMALIZED coordinates (0-1) --
    // We must scale them up to pixel coordinates (width/height)
    // The ctx.scale above handles the Zoom.
    ...
    polygons.forEach(poly => {
      ...
      ctx.lineWidth = 2 / view.scale; // Keep line width constant visually
      ctx.stroke();
    });
    ...
    if (currentPoly.length > 0) {
      ...
      ctx.setLineDash([5 / view.scale, 5 / view.scale]); // Scale dash
      ctx.stroke();
      ctx.setLineDash([]);
      ...
      currentPoly.forEach(p => {
        ...
        ctx.fillStyle = '#fff';
        ctx.beginPath();
        ctx.arc(sp.x, sp.y, 4 / view.scale, 0, Math.PI * 2); // Scale dot
        ctx.fill();
        ctx.stroke();
      });
    }
    ...
    if (calibrationLine) {
       ...
       ctx.beginPath();
       ctx.arc(s1.x, s1.y, 5 / view.scale, 0, Math.PI * 2);
       ctx.arc(s2.x, s2.y, 5 / view.scale, 0, Math.PI * 2);
       ctx.fill();
       ...
       if (scaleFactor) {
           ...
           ctx.fillStyle = 'white';
           ctx.font = `bold ${14/view.scale}px sans-serif`; // Scale font
           ...
           ctx.fillStyle = 'white';
           ctx.fillText(text, mid.x - tm.width/2, mid.y - (5/view.scale)); // Approx centering
       }
    }
  ...
  const getWrapperStyle = () => {
      ...
      // If image is wider relative to container -> constrained by width
                     ...
                     {polygons.map((poly, i) => {
                         ...
                         return (
                             <button
                                ...
                                style={{ 
                                    // Position needs to respect zoom/pan
