...
/**
 * LAWN ESTLY - v3.9.16
 * - Design Update: 3-Blade Grass Icon scaled up 30% and moved closer together.
 * - Logic Check: Enforced "Move Tool Only" for Zoom/Pan on desktop.
 * - Version Bump: 3.9.16
 */

// --- Helper Components ---

// Custom Grass Icon Component for Header (3 Blades, Large, Tight Cluster)
const GrassIcon = ({ className }) => (
  <svg 
    ...
    className={className}
  >
    {/* Left Blade: Large, flat top, base very close to center */}
    <path d="M26 60 L 14 10 H 26 L 30 60 Z" />
    
    {/* Center Blade: Tallest, overlapping bases */}
    <path d="M32 60 L 28 4 H 38 L 34 60 Z" />
    
    {/* Right Blade: Large, flat top, base very close to center */}
    <path d="M38 60 L 40 14 H 52 L 40 60 Z" />
  </svg>
);
...
const FaviconManager = () => {
  useEffect(() => {
    // Custom Grass Icon (Monocot) - 3 Blades, Large, Tight Cluster
    const svg = `
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" fill="none">
        <rect width="64" height="64" rx="16" fill="#ecfdf5"/>
        
        <!-- Left Blade -->
        <path d="M26 60 L 14 10 H 26 L 30 60 Z" fill="#059669" />
        
        <!-- Center Blade -->
        <path d="M32 60 L 28 4 H 38 L 34 60 Z" fill="#059669" />
        
        <!-- Right Blade -->
        <path d="M38 60 L 40 14 H 52 L 40 60 Z" fill="#059669" />
      </svg>
    `.replace(/\n/g, '').replace(/#/g, '%23');
...
export default function App() {
  ...
  const handleWheel = (e) => {
      ...
      if (mode !== 'PAN') {
          // If we want to block scrolling, we can do e.preventDefault() if event was non-passive
          // but React synthetic events make that hard.
          // Just return.
          e.stopPropagation();
          return;
      }
      
      // Stop event from bubbling to page scroll
      e.stopPropagation();
